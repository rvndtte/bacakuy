...
app.post('/api/auth/register', (req, res) => { const name = String(req.body.name || '').trim(); const email = String(req.body.email || '').trim().toLowerCase(); const password = String(req.body.password || ''); if (!name || !email || password.length < 6) return res.status(400).json({ error: 'Nama, email, dan password minimal 6 karakter wajib diisi' }); try { const avatar = name.split(/\s+/).map(part => part[0]).join('').slice(0, 2).toUpperCase(); const result = db.prepare('INSERT INTO auth_users (name, email, password_hash, role, avatar) VALUES (?, ?, ?, ?, ?)').run(name, email, hashPassword(password), 'Pembaca aktif', avatar); const token = crypto.randomBytes(32).toString('hex'); db.prepare('INSERT INTO auth_tokens (token, user_id) VALUES (?, ?)').run(token, result.lastInsertRowid); res.status(201).json({ token, user: publicUser({ id: result.lastInsertRowid, name, role: 'Pembaca aktif', avatar }) }) } catch { res.status(409).json({ error: 'Email sudah terdaftar' }) } })
app.post('/api/auth/login', (req, res) => { const email = String(req.body.email || '').trim().toLowerCase(); const password = String(req.body.password || ''); const user = db.prepare('SELECT * FROM auth_users WHERE email = ?').get(email); if (!user || !verifyPassword(password, user.password_hash)) return res.status(401).json({ error: 'Email atau password salah' }); const token = crypto.randomBytes(32).toString('hex'); db.prepare('INSERT INTO auth_tokens (token, user_id) VALUES (?, ?)').run(token, user.id); res.json({ token, user: publicUser(user) }) })
app.get('/api/auth/me', (req, res) => { const session = db.prepare('SELECT users.* FROM auth_tokens JOIN users ON users.id = auth_tokens.user_id WHERE auth_tokens.token = ?').get(getToken(req)); if (!session) return res.status(401).json({ error: 'Sesi tidak valid' }); res.json(publicUser(session)) })
app.post('/api/auth/logout', (req, res) => { db.prepare('DELETE FROM auth_tokens WHERE token = ?').run(getToken(req)); res.sendStatus(204) })
